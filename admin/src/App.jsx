        ...
        {visibleProducts.map((product) => {
          ...
          return (
            <article className="product-row" key={product.id}>
              ...
              <div className="row-actions"><button className="secondary-button" onClick={() => onEdit(product.id)} type="button">Edit</button>{product.archivedAt ? <button className="secondary-button" onClick={() => onRestore(product.id)} type="button">Restore</button> : <button className="secondary-button" onClick={() => onArchive(product.id)} type="button">Archive</button>}<button className="icon-button danger-button" onClick={() => onDelete(product.id)} title="Permanently delete product" type="button"><Trash2 size={17} /></button><a className="icon-button" href={`https://ballai.dev/#store/${product.slug}`} rel="noopener noreferrer" target="_blank" title="View live product"><ExternalLink size={17} /></a></div>
            </article>
          );
        })}
      </section>
...
export default function App() {
  ...
  const deleteListing = async (productId) => { const product = products.find((item) => item.id === productId); const name = productInfo(productId, product).title; if (!window.confirm(`Permanently delete ${name}? This removes the listing, revisions, media, and promotion data. This cannot be undone. Use Archive if you may want to restore it later.`)) return; try { await adminApi.deleteProduct(productId); setProducts((current) => current.filter((product) => product.id !== productId)); } catch (requestError) { setError(requestError.message); } };
  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main"><ErrorBanner message={error} onRetry={loadCore} />{newListing ? <NewListing onCancel={() => setNewListing(false)} onCreated={createListing} /> : editingProduct ? <ProductEditor key={`${editingProduct.id}-${editingProduct.draft.updatedAt}`} onBack={() => setEditingId(null)} onChanged={updateProduct} product={editingProduct} /> : section === "dashboard" ? (dashboard ? <Dashboard data={dashboard} onEdit={openEditor} onEndAllSales={() => setEmergencyOpen(true)} products={products} /> : <p>Loading dashboard...</p>) : section === "products" ? <Products onArchive={archiveListing} onDelete={deleteListing} onEdit={openEditor} onNew={() => setNewListing(true)} onRestore={restoreListing} products={products} /> : section === "promotions" ? <Promotions onEdit={openEditor} products={products} /> : <Analytics data={analytics} onRange={setAnalyticsRange} range={analyticsRange} />}</main>
